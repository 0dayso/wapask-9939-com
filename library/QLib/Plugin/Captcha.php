 ...
 * @since 1.0 
 **/
class QLib_Plugin_Captcha extends Zend_Controller_Plugin_Abstract {
	static $V = array(
		"a", 
		"e", 
		"i", 
	...
	protected function _generateImage($word) {
		if (!extension_loaded("gd")) {
			throw new QLib_Exception("Image CAPTCHA requires GD extension");
		}
		if (!function_exists("imagepng")) {
			throw new QLib_Exception("Image CAPTCHA requires PNG support");
		}
		if (!function_exists("imageftbbox")) {
			throw new QLib_Exception("Image CAPTCHA requires FT fonts support");
		}
		$font = $this->getFont();
		if (empty($font)) {
			throw new QLib_Exception("Image CAPTCHA requires font");
		}
		$w = $this->getWidth();
		$h = $this->getHeight();
