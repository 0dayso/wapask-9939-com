...
<head>
    <meta charset="utf-8">
    <title>绑定手机_久久问医</title>

    <meta name="baidu-site-verification" content="wRAn557dE3" />
    <meta name="keywords" content="久久问医,绑定手机页面" />
    <meta name="description" content="久久问医的绑定手机页面，提供绑定手机信息." />
...
<form action="/doctor/bindusertel" method="post" class="formst suinf">

    <div id="not_bind" style="display: <{if empty($is_binding_mobile) }>block<{else}>none<{/if}>;">
        <div class="indis">
    ...
    <div id="binded" style="display: <{if !empty($is_binding_mobile) }>block<{else}>none<{/if}>;">
        <div class="indis">
            <input type="text" placeholder="手机号" value="<{$telephone}>" disabled = disabled>
            <a></a>
        </div>
        <input type="submit" class="subs" value="重新绑定" id="rebind">
    </div>
